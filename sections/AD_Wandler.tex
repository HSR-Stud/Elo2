...
\begin{tabular}{p{0.31\textwidth}|l}
 ...
 \multirow{3}{12cm}{
    \begin{gather}
      ...
      \cdot C_{i}}(V_{in}-V_{GNDA})dt+V_{GNDA} \quad \text{für }0<t<T_{int}\\
      V_{intmax}(V_{in})=\frac{-1}{R_{int}
      \cdot C_{i}}(V_{in}-V_{GNDA})T_{int}+V_{GNDA}\\
      V_{abint}(t)=V_{intmax}-\frac{1}{R_{int}
      \cdot C_{i}}\cdot (V_{refn}-V_{GNDA}) \cdot t \quad \text
      {für }T_{int}<t<T\\
      t_{abint}=\frac{(V_{GNDA}-V_{in}) \cdot T_{int}}{V_{refn}-V_{GNDA}}\\
      \text{Für }V_{GNDA}=0\text{ :}\\
